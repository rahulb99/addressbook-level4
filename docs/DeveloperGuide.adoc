...
=== Find feature
==== Current Implementation
There is a `FindCommand` class which implements this feature.
===== Working
Step 1: `FindCommand` 's execute method passes the predicate (which is basically input arguments) of type `ExpenseContainsKeywordsPredicate` to `Model` 's `find` method.
Step 2: `ModelManager` which implements the `Model` interface, provides the implementation for the `find` method.
Step 3: We then filter the expenses by setting the predicate and `ExpenseContainsKeywordsPredicate` then performs the underlying work - finds all the expenses according to input arguments.
Step 4: We finally list out the filtered expenses.
...
|`* *` |user |see the report of my spending |know how much I spend
...
2. User enters a add command with the name of item, price, categories and date
